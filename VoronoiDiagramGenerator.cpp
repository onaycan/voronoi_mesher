...
VoronoiDiagramGenerator::~VoronoiDiagramGenerator()
{
	//cout<<"In ~VoronoiDiagramGenerator()"<<endl;
	reset();
...
{
	//cout<<"In VoronoiDiagramGenerator::reset()"<<endl;
	cleanup();
	cleanupEdges();
	...
	//cout<<"At end of VoronoiDiagramGenerator::reset()"<<endl;
											  ...
											  float maxX, float minY, float maxY, float minDist, bool genVertexInfo)
{
	...
	if(sites == 0)
	{
		//cout<<"generateVoronoi returning false 1";
		return false;
	}
	...
	//cout<<"About to call voronoi("<<triangulate<<")";
...
void VoronoiDiagramGenerator::cleanup()
{
	...
	//cout<<"At the end of cleanup";
}

void VoronoiDiagramGenerator::cleanupEdges()
{
	//cout<<"At start of cleanupEdges"<<endl;
	...
	delaunayEdges = 0;
	//cout<<"At end of cleanupEdges"<<endl;
}
...
void VoronoiDiagramGenerator::pushDelaunayGraphEdge(float x1, float y1, float x2, float y2)
{
	...
	{
		//cout<<"Skipping line of length "<<(x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)<<" because minDistanceBetweenSites = "<<minDistanceBetweenSites;
		return;
	}
	//cout<<"NOT Skipping line of length "<<(x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)<<" because minDistanceBetweenSites = "<<minDistanceBetweenSites;
	...
	//cout<<"Pushed Delaunay edge ("<<x1<<","<<y1<<") -> ("<<x2<<","<<y2<<"), now delaunayEdges = "<<delaunayEdges;
}
...
void VoronoiDiagramGenerator::out_bisector(struct Edge *e)
{
	if(genDelaunay)
	{
		...
		//cout<<"Pused Delaunay Edge ("<<e->reg[0]->coord.x<<","<<e->reg[0]->coord.y<<") -> ("<<e->reg[1]->coord.x<<","<<e->reg[1]->coord.y<<")";
...
bool VoronoiDiagramGenerator::voronoi(bool genVertexInfo)
{
	...
	if(!retval)
	{
		//cout<<"voronoi returning false 1";
		return false;
	}
	
	newsite = nextone();

	//cout<<"About to go into the infinite while loop";
	long counter = 0;
	while(1)
	{
		//cout<<++counter;
	...
	cout<<"Voronoi: after finishing the voronoi diagram - about to get the vertices"<<endl;
	cout<<"sizeOfVertexLinks = "<<sizeOfVertexLinks<<endl;
//	count the total number of 
	...
	if(genVertexInfo)
	{
		...
		cout<<"After counting the size of the final vertices = "<<sizeOfFinalVertices<<endl;
		...
		//cout<<"Just before generateVertexLinks()"<<endl;

		generateVertexLinks();
		//cout<<"Just after generateVertexLinks()"<<endl;

	}
	
	//cout<<"After generateVertexLinks()"<<endl;
	cleanup();
...
void VoronoiDiagramGenerator::insertVertexAddress(long vertexNum, struct Site* address)
{
	...
	while(vertexNum > sizeOfVertices - 1)
	{
		...
		//cout<<"Grew vertices to size "<<sizeOfVertices<<" and vertices = "<<vertices<<endl;
	}
...
void VoronoiDiagramGenerator::insertVertexLink(long vertexNum, long vertexLinkedTo)
{
	if(vertexLinks == 0)
	{
		//cout<<"Initialising vertexlinks for the first time";
	...
	{
		//cout<<"Resizing the array to "<<sizeOfVertexLinks + 4000<<endl;
		//cout<<endl;
		vertexLinks = (Point3*)realloc(vertexLinks,(sizeOfVertexLinks+4000)*sizeof(Point3));
		if(vertexLinks == 0)//cout<<"Error - realloc failed, vertexLinks == 0"<<endl;
		sizeOfVertexLinks += 4000;
	...
	//cout<<"insertVertexLink exit"<<endl;
}
...
void VoronoiDiagramGenerator::generateVertexLinks()
{
	...
	if(finalVertexLinks != 0)
	{
		cout<<"Just before freeing finalVertexLinks"<<endl;
		free(finalVertexLinks);
		finalVertexLinks = 0;
	}
	...
	if(vertices == 0)
	{
		cout<<"vertices is zero, not doing anything in generateVertexLinks()"<<endl;
		return;
	}

	sizeOfFinalVertexLinks = (sizeOfVertices>sizeOfVertexLinks) ? sizeOfVertices : sizeOfVertexLinks;

	cout<<"sizeOfFinalVertexLinks = "<<sizeOfFinalVertexLinks<<",sizeOfVertexLinks = "<<sizeOfVertexLinks<<endl; 
	...
	cout<<"Created vertexLinks array of size "<<sizeOfFinalVertexLinks<<endl;
	cout<<"finalVertexLinks = "<<finalVertexLinks<<endl;
	...
	cout<<"sizeOfVertices = "<<sizeOfVertices<<endl;
	cout<<"sizeOfVertexLinks = "<<sizeOfVertexLinks<<endl;
	cout<<"count1vertices and count3vertices are of size "<<sizeOfFinalVertexLinks<<endl;
	cout<<"finalVertexLinks is of size "<<sizeOfFinalVertexLinks<<endl;
	
	long count1counter = 0, count3counter = 0;
	long count3links[3] = {-1};
	
	//cout<<"Before copying "<<sizeOfVertices<<" coordinates into finalVertexLinks"<<endl;
	for(i = 0; i< sizeOfVertices; i++)
	{		
		//cout<<"copying "<<i<<" coords,vertices[i]="<<vertices[i]<<endl;
		if(vertices[i] == 0)
			break;
	...
	//cout<<"After copying "<<i+1<<" coordinates into finalVertexLinks"<<endl;
	...
	//cout<<"About to process the "<<count1counter<<" count 1 (leaf) vertices"<<endl;
	...
	//cout<<"Finished processing the "<<count1counter<<" count 1 (leaf) vertices";
	//cout<<"About to process the "<<count3counter<<" count 3 (non-leaf) vertices"<<endl;
	...
	{
		//cout<<i<<", count3vertices["<<i<<"] = "<<count3vertices[i];
		...
		//cout<<"after count3links, finalVertexLinks[count3vertices[i]].count = "<<finalVertexLinks[count3vertices[i]].count<<endl;

//		//coutCODE		if(finalVertexLinks[count3vertices[i]].count > 2 || finalVertexLinks[count3vertices[i]].count < 0)
//		//coutCODE		{
//		//coutCODE			//cout<<"Error: finalVertexLinks[count3vertices[i]].count = "<<finalVertexLinks[count3vertices[i]].count;
//		//coutCODE			return;
//		//coutCODE		}
		...
		//cout<<"after finalVertexLinks[count3vertices[i]]"<<endl;
		...
		for(j = 0; j< 3; j++)//process each of the vertices that the current one is linked to
		{
			...
			{
				//cout<<"Error, currentVertex = "<<currentVertex<<" and sizeOfVertexLinks = "<<sizeOfVertexLinks;
				//cout<<"On element "<<i+1<<" of "<<count3counter;
	...
	//cout<<"Finished processing the count 3 vertices"<<endl;
}
